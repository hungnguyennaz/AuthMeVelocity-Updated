...
public class VelocityMessageListener implements SettingsDependent {
    ...
    private void handleOnLogout(final ByteArrayDataInput in) {
        ...
        final Optional<Player> player = authPlayer.getPlayer();
        player.ifPresent(p -> AuthMeVelocity.getInstance().getProxy().getServer(sendOnLogoutTarget)
            .ifPresent(s -> p.createConnectionRequest(s).connect()));
    }
